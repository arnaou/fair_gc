...
def objective(trial):
    ...
    params = {
        ...
        'max_features': trial.suggest_categorical('max_features', ['sqrt', 'log2', None]),  # Corrected options
...
def predict_with_std(model, X):
    predictions = np.zeros((len(X), len(model.estimators_)))
...
y_pred, y_std = predict_with_std(best_model, X_test)
print("\nPrediction Examples (with uncertainty):")
for i in range(min(5, len(y_test))):
    print(f"True: {y_test[i][0]:.4f}, Predicted: {y_pred[i]:.4f} ± {2*y_std[i]:.4f}")
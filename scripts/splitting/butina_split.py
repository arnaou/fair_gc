...
from src.splits import split_molecules_by_clusters
import numpy as np
...
val_size = 0.15
train_size = 0.7
# conduct Butina clustering
smi_list = df_mg['SMILES'].to_list()

train_smiles, val_smiles, test_smiles = split_molecules_by_clusters(smi_list, [train_size, val_size, test_size], cutoff=0.85, random_seed=42)
#%%
df_train = df_mg[df_mg['SMILES'].isin(train_smiles)].copy()
df_val = df_mg[df_mg['SMILES'].isin(val_smiles)].copy()
df_test = df_mg[df_mg['SMILES'].isin(test_smiles)].copy()
...
df_final.to_excel('../data/processed/'+prop_tag+'/'+prop_tag+'_'+split_type+'_processed.xlsx', index=False)
# # Set seed for reproducibility
# seeds = [42, 7, 3, 109, 59]   # You can choose any integer value

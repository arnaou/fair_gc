...
parser.add_argument('--split_type', type=str, default='butina_min', help='path for storing the model')
...
path_2_result = args.path_2_result+ args.property+'/classical/'+args.property+'_min_result.xlsx'
path_2_model = args.path_2_model+args.property+'/classical/'+args.property+'/'+'min'
...
# split the data
df_train = df[df['required']==True]
df_test = df[df['required']!=True]
...
step_res1 = least_squares(Fobj_fog, theta01, jac='3-point', bounds=(-np.inf, np.inf),loss='soft_l1',
                        args=(X1_train, y_train['true'], args.property, 'res'), verbose=2)

step_res2 = least_squares(Fobj_sog, theta02, jac='3-point', bounds=(-np.inf, np.inf),loss='soft_l1',
                        args=(X2_train, y_train['true'], args.property, X1_train, step_res1.x, 'res'), verbose=2)

step_res3 = least_squares(Fobj_tog, theta03, jac='3-point', bounds=(-np.inf, np.inf), loss='soft_l1',
                        args=(X3_train, y_train['true'], args.property, X1_train, step_res1.x, X2_train, step_res2.x, 'res'), verbose=2)
...
df_result = df.copy()
y_true = np.hstack((y_train['true'], y_test['true']))
y_step_pred = np.hstack((y_train['step_pred'], y_test['step_pred']))

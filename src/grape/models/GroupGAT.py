...
class SingleHeadOriginLayer(nn.Module):
    ...
    def forward(self, data):
        ...
        origin_graph_features = self.AttentiveEmbedding(d)
...
class SingleHeadFragmentLayer(nn.Module):
    ...
    def forward(self, data):
        d = Data(data.x, data.edge_index, data.edge_attr, data.batch)
        d.batch = data.batch
        frag_features = self.AttentiveEmbedding(d)
...
class FragmentChannel(nn.Module):
    def __init__(self, net_params):
        super().__init__()
        self.embedding_node_lin = nn.Sequential(
            nn.Linear(net_params["node_in_dim"], net_params["L2_hidden_dim"], bias=True),
            nn.BatchNorm1d(net_params["L2_hidden_dim"]),
            nn.LeakyReLU()
        )
        self.embedding_edge_lin = nn.Sequential(
            nn.Linear(net_params['edge_in_dim'], net_params["L2_hidden_dim"], bias=True),
            nn.BatchNorm1d(net_params["L2_hidden_dim"]),
            nn.LeakyReLU()
        )
        self.fragment_heads = nn.ModuleList(
            [SingleHeadFragmentLayer(net_params) for _ in range(net_params['num_heads'])])
...
class SingleHeadJunctionLayer(nn.Module):
    def __init__(self, net_params):
        super().__init__()
        self.project_motif = nn.Linear(net_params['L2_hidden_dim'] + net_params['L3_hidden_dim'],
                                       net_params['L3_hidden_dim'], bias=True)
        self.AttentiveEmbedding = AFP(
            node_in_dim=net_params["L3_hidden_dim"],
            edge_in_dim=net_params["L3_hidden_dim"],
            # seems like edges also sent to this dimension: is it correct? (TODO)
            hidden_dim=net_params["L3_hidden_dim"],  # internal AFP dim
            num_layers_atom=net_params['L3_layers_atom'],
            num_layers_mol=net_params['L3_layers_mol'],
            dropout=net_params['L3_dropout'],
            out_dim=net_params["L3_hidden_dim"],
            regressor=False,
        )

    def forward(self, data):
        x_projected = self.project_motif(data.x)
        d = Data(x_projected, data.edge_index, data.edge_attr, data.batch)
        d.batch = data.batch
...
class JT_Channel(nn.Module):
    def __init__(self, net_params):
        ...
        self.embedding_edge_lin = nn.Sequential(
            nn.Linear(net_params['edge_in_dim'], net_params['L3_hidden_dim'], bias=True),
            nn.BatchNorm1d(net_params['L3_hidden_dim']),
            nn.LeakyReLU()
        )
    ...
    def forward(self, batch, motif_nodes):
        ...
        batch.x = torch.cat([batch.x, motif_nodes],
                            dim=-1)  # `(L2_hidden_dim + L3_hidden_dim) * num_frags` (or permuted idk)
        junction_graph_heads_out = []
        junction_attention_heads_out = []
        for single_head in self.junction_heads:
            single_head_new_graph = single_head(batch)
...
class GCGAT_v4pro(nn.Module):
    ...
    def forward(self, data, get_descriptors=False, global_feature=None):
        ...
        super_new_graph= self.junction_module(junction_data, motif_nodes)  # super_attention_weight

        # sum features for motif graph (akin to dgl.sum_nodes)
        num_mols = int(junction_data.batch.max().item()) + 1#len(junction_data.batch.unique(dim=0, ))
